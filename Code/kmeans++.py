...
def plot_cluster(data_mat, cluste_assment, centroid):
	...
	for i, col in zip(range(k), colors):
		...
		plt.plot(per_data_set[:, 0], per_data_set[:, 1], 'o',
				 markerfacecolor=tuple(col),markeredgecolor='k', markersize=5)
	for i in range(k):
...
if __name__ == '__main__':
	...
	# k_num = 7
	sse_list = []
	for k_num in tqdm(range(2, 15)):
		data_mat = mat(load_data("AVE_FV.txt"))
		centroid, cluster_assment = kpp_Means(data_mat, k_num)
		sse = sum(cluster_assment[:,1])
		sse_list.append(sse)
		# print("k_num is {}, sse is {}".format(k_num, sse))
		# plot_cluster(data_mat, cluster_assment, centroid)
	print(sse_list)
	print(min(sse_list))
	print(sse_list.index(min(sse_list)))
